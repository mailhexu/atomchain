...
from phonopy.file_IO import write_FORCE_CONSTANTS, write_disp_yaml, parse_FORCE_SETS
...
def _setup_phonon(
    ...
    write_disp_yaml(
        disps, supercell0, filename=os.path.join(phonon_save_dir, "disp.yaml")
    )
...
def _load_forces_from_pickle(phonon, restart, phonon_save_dir):
    ...
    print(f"[Phonopy] Reading forces from existing '{pickle_file}' file")
    with open(pickle_file, "rb") as myfile:
...
def _create_force_calculator(
    supercells,
    calc,
    is_mag,
    sc_mag,
    mask_force,
    prepare_initial_wavecar,
    func,
    func_args,
    phonon_save_dir,
):
    ...
    def calc_force(iscell):
        ...
        if prepare_initial_wavecar:
            supercell0_dir = os.path.join(phonon_save_dir, "SUPERCELL0")
            os.system(
                "ln -s %s %s"
                % (
                    os.path.abspath(os.path.join(supercell0_dir, "WAVECAR")),
            ...
            os.system(
                "cp %s %s"
                % (
                    os.path.abspath(os.path.join(supercell0_dir, "siesta.DM")),
...
def _calculate_forces(
    ...
    if parallel:
        ...
        for iscell in range(iskip, len(supercells)):
            print(
                f"[Phonopy] Calculating forces for supercell {iscell + 1}/{len(supercells)}"
            )
...
def _save_results(phonon, phonon_save_dir):
    """Save phonon results to files."""
    force_constants = phonon.force_constants
    write_FORCE_CONSTANTS(
        force_constants, filename=os.path.join(phonon_save_dir, "FORCE_CONSTANTS")
    )

    with open(os.path.join(phonon_save_dir, "phonon.pickle"), "wb") as myfile:
        pickle.dump(phonon, myfile)

    phonon.save(
        filename=os.path.join(phonon_save_dir, "phonopy_params.yaml"),
...
def calculate_phonon(
    ...
    **func_args,
    ...
    Args:
        ...
        phonon_save_dir: Directory for phonon output files (default: "phonon_save")
    ...
    # Create phonon save directory if it doesn't exist
    if not os.path.exists(phonon_save_dir):
        os.makedirs(phonon_save_dir)
        print(f"[Phonopy] Created output directory: {phonon_save_dir}")
    ...
    if forces_set_file is not None:
        ...
        set_of_forces, iskip, is_complete = _load_forces_from_pickle(
            phonon, restart, phonon_save_dir
        )

        if is_complete:
            # All forces loaded from pickle
            phonon.produce_force_constants(forces=np.array(set_of_forces))
            _save_results(phonon, phonon_save_dir)
            return phonon

        # Need to calculate forces (some or all)
        if prepare_initial_wavecar and skip is None:
            _prepare_initial_wavecar(supercell0, calc, is_mag, sc_mag, phonon_save_dir)
...
def compute_phonon_at_qpoints(qpoints, phonopy_yaml="phonon_save/phonopy_params.yaml"):
    """
    ...
    for iq, qpt in enumerate(qpoints):
        ...
        print(
            f"[Phonopy] Q-point {iq + 1}/{nqpoints}: {qpt} - "
