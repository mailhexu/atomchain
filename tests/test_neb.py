...
def test_interpolate_path_periodic_mic():
    ...
    assert (
        images[1].positions[0, 0] < images[0].positions[0, 0]
...
def test_neb_file_not_found():
    ...
    with pytest.raises(FileNotFoundError):
        calculate_neb("nonexistent_initial.vasp", "nonexistent_final.vasp", quiet=True)
...
def test_neb_cli_help():
    ...
    """
    result = subprocess.run(["mlneb", "--help"], capture_output=True, text=True)
